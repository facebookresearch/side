...
class DPR(Retriever):
    ...
    def __init__(self, name, cfg):
        ...
        if os.path.exists(f"{cfg.retriever.model_file}.generate_embeddings"):
            ...
            raise Exception(f"Checkpoint {cfg.retriever.model_file}.generate_embeddings or {cfg.retriever.model_file} doesn't exist.")
    ...
    def run(self):
        ...
        top_ids_and_scores = self.retriever.get_top_docs(
            questions_tensor.numpy(), self.cfg.n_docs, filter_pos=4, filter_value="1",
        )
