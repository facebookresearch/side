...
CheckpointState = collections.namedtuple(
    ...
    [
        ...
        "offset",
        "epoch",
        "best_validation_loss_result",
        "best_validation_acc_result",
        "encoder_params",
    ],
)


def setup_fp16_and_distributed_mode(
    model: nn.Module,
...
def get_model_file(args, file_prefix) -> str:
    if args.resume.model_file and os.path.exists(args.resume.model_file):
        return args.resume.model_file

    out_cp_files = glob.glob(os.path.join(args.output_dir, file_prefix + "*[0-9]")) if args.output_dir else []
