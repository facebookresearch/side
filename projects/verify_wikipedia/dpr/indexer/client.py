...
from distributed_faiss import rpc
...
class IndexClient:
    ...
    def __init__(self, server_list_path: str, cfg_path: Optional[str] = None):
        ...
        self.index_rank_to_id = {index_rank: index_id for index_id, index_rank in enumerate(index_ranks)}
    ...
    def read_server_list(
        ...
        while True:
            ...
            if num_servers != len(res):
                print(msg + f" Waiting {round(initial_timeout * 100) / 100} seconds for servers to load...")
        ...
        assert num_servers == len(res), msg + f" Timed out after waiting {round(time_waited * 100) / 100} seconds"
        return res
    ...
    def load_index(
        ...
        def setup_cfg(cfg: Optional[IndexCfg]):
            if cfg is None:
                config_paths = self.pool.map(lambda idx: idx.get_config_path(index_id), self.sub_indexes)
    ...
    def add_index_data(
        ...
        self.sub_indexes[cur_server_id].add_index_data(index_id, embeddings, metadata, train_async_if_triggered)
    ...
    def search(self, query, topk: int, index_id: str, return_embeddings: bool = False) -> Tuple[np.ndarray, List]:
        ...
        results = self.pool.imap(lambda idx: idx.search(index_id, query, topk, return_embeddings), self.sub_indexes)
    ...
    def search_with_filter(
        ...
        def _do_filter(scores: np.array, results_meta: List[List[Tuple]]):
            ...
            for i, meta_list in enumerate(results_meta):
                ...
                if len(sample_filtered_scores) > 0:
                    new_scores.append(np.concatenate([s.reshape(-1, 1) for s in sample_filtered_scores], axis=0))
